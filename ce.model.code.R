...
for (i in 1:4){
    ...
    # Load the data
    pr <- readRDS(paste0(path, gcm, "/pr.dd.pi.", gcm, ".", rcm, ".rcp85.", wl[level.wl], ".rds"))
    tmax <- readRDS(paste0(path, gcm, "/tmax.dd.pi.", gcm, ".", rcm, ".rcp85.", wl[level.wl], ".rds"))
    ...
    saveRDS(frecuency, paste0(path, gcm, "/ce.frecuency.pi.", gcm, ".", rcm, ".rcp85.", wl[level.wl], ".rds"))

    print("Trends")
    trends <- fun.trends.frecuency(ce=frecuency, hottest.month=hottest.month, mask=mask)
    saveRDS(trends, paste0(path, gcm, "/ce.trends.frecuency.pi.", gcm, ".", rcm, ".rcp85.", wl[level.wl], ".rds"))

    print("Duration")
    duration <- fun.duration(frecuency, hottest.month, racha, years[[level.wl]])
    saveRDS(duration, paste0(path, gcm, "/ce.duration.pi.", gcm, ".", rcm, ".rcp85.", wl[level.wl], ".rds"))

    print("Severity")
    severity <- fun.severity(hottest.month, pr.obs, tmax.obs, pr.transformed, tmax.transformed)
    saveRDS(severity, paste0(path, gcm, "/ce.severity.pi.", gcm, ".", rcm, ".rcp85.", wl[level.wl], ".rds"))

    print("Categories")
    categories <- fun.categories(severity, hottest.month)
    saveRDS(categories, paste0(path, gcm, "/ce.categories.pi.", gcm, ".", rcm, ".rcp85.", wl[level.wl], ".rds"))
}
